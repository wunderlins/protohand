// Automatically generated file. Edit error.html and run 
// 'make ini' to update this documentation!

char* error_str = ""
"<!doctype html>\n"
"\n"
"<html lang=\"en\">\n"
"<head>\n"
"	<meta charset=\"utf-8\">\n"
"	<title>ph Error</title>\n"
"	<meta name=\"description\" content=\"\">\n"
"	<meta name=\"author\" content=\"\">\n"
"	<style>\n"
"		body {font-family: arial, helvetica, sans;}\n"
"		h1 {border-bottom: 1px solid gray;}\n"
"		.detail {display: none;}\n"
"	</style>\n"
"</head>\n"
"\n"
"<body>\n"
"	<h1>Error: <span id=\"code\"></span></h1>\n"
"	<p id=\"description\"></p>\n"
"	\n"
"	<div>\n"
"		<div class=\"detail\" id=\"err1\">Argument 1 missing.</div>\n"
"		<div class=\"detail\" id=\"err2\">Failed to find current directory. <code>getcwd()</code> or <code>realpath()</code> failed to resolve the app directory.</div>\n"
"		<div class=\"detail\" id=\"err3\">Failed to create <code>ph.ini</code>, permission denied.</div>\n"
"		<div class=\"detail\" id=\"err4\">Failed to create <code>ph.ini</code>, unable to write to file.</div>\n"
"		<div class=\"detail\" id=\"err5\">Failed to parse ini file.</div>\n"
"		<div class=\"detail\" id=\"err6\">Wrong path or program is not executable.</div>\n"
"		\n"
"		<div class=\"detail\" id=\"err128\">Failed to parse URI, protocol missing.</div>\n"
"		<div class=\"detail\" id=\"err129\">Failed to parse URI, authority missing.</div>\n"
"		<div class=\"detail\" id=\"err130\">Failed to parse URI, error in query string.</div>\n"
"		<div class=\"detail\" id=\"err131\">Failed to parse URI, error in fragment.</div>\n"
"	</div>\n"
"	<br/>\n"
"	<hr size=\"1\"/>\n"
"	<em><small>Check the error log for more information.</small></em>\n"
"	\n"
"</body>\n"
"<script type=\"text/javascript\">\n"
"var code = window.location.search.slice(1);\n"
"document.getElementById(\"code\").innerHTML = code;\n"
"try {\n"
"	document.getElementById(\"err\"+code).style.display = \"block\";\n"
"} catch (e) {;}\n"
"</script>\n"
"</html>\n""";
